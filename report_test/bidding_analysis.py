...
def getorderreson(mydit):
    ...
    print 111111111223232123132132121312222222222
    print count
    # 相同原因未出广告订单列表
    orderlist=[]
    alldit={}
...
def orderbylognew(zclk):
    ...
    for hit in res["hits"]["hits"]:
        ...
        # print 222222
        # 不出广告订单列表
        print str(hit["_source"]["message"]).split("bid:")[1].split(", wad:")[0].split("fad:")[1]
...
def yiqifaes(zclk):
    es=Elasticsearch([{"host":"221.122.127.227"}],port=9200);
    # res = es.search(index="logstash-voyagerjavalog-*", body={"query": {"zclk":"B3J3JDC11HP5D1KDOM"}, "_source": "BID_INFO"})
    body={
          "query": {
            "match_all": {}
    ...
    # body['query']["bool"]['must'][0]['query_string']['query']='path:bidding  AND message:"{0}"'.format(zclk)
    # print body
    # res = es.search(index="yiqifa_product_online/product_post/_search", body=body)
    res = es.search(index="yiqifa_product_online",doc_type='product_post', body=body)
    print res
    tmplist=[]
    for hit in res["hits"]["hits"]:
        tmpdict={}
        ...
        # print 5555555555555555555555555555
        # 时间
        mytime=str(hit["_source"]["post_publish_time"])
        print mytime
        print type(mytime)
        tmplist.append(tmpdict)
    return tmplist
